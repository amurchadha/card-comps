/**
 * Cardboard Connection Soccer Checklist Scraper
 */
...
function extractSetUrls(html: string): string[] {
  const urls: string[] = [];
  const linkRegex = /href="(https:\/\/www\.cardboardconnection\.com\/\d{4}-[^"]+soccer[^"]*)"/gi;
  let match;
...
function parseSetMetadata(url: string, html: string) {
  ...
  if (lower.includes('panini') || lower.includes('prizm') || lower.includes('donruss') || lower.includes('select') || lower.includes('mosaic') || lower.includes('immaculate') || lower.includes('national')) {
    manufacturer = 'Panini';
  } else if (lower.includes('topps') || lower.includes('bowman') || lower.includes('chrome') || lower.includes('merlin') || lower.includes('match-attax')) {
    manufacturer = 'Topps';
  } else if (lower.includes('leaf')) {
    manufacturer = 'Leaf';
  } else if (lower.includes('upper-deck')) {
    manufacturer = 'Upper Deck';
  } else if (lower.includes('futera')) {
    manufacturer = 'Futera';
  }

  let productLine = '';
  const patterns = ['prizm', 'select', 'mosaic', 'donruss', 'optic', 'national-treasures', 'flawless', 'immaculate', 'chronicles', 'obsidian', 'chrome', 'finest', 'match-attax', 'merlin', 'topps'];
...
async function scrapeYear(year: string) {
  console.log(`\n=== Scraping ${year} soccer sets ===`);
  const yearUrl = `${BASE_URL}/sports-cards-sets/soccer-card-sets/${year}-soccer-cards`;
  ...
  for (const setUrl of setUrls) {
    ...
    if (existingSet) {
      ...
      const { data: newSet, error } = await supabase.from('card_sets').insert({
        name: metadata.name, year: metadata.year, sport: 'soccer',
...
async function main() {
  console.log('Soccer Checklist Scraper\n');
