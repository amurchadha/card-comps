/**
 * Cardboard Connection Baseball Checklist Scraper
 */
...
function extractSetUrls(html: string): string[] {
  const urls: string[] = [];
  const linkRegex = /href="(https:\/\/www\.cardboardconnection\.com\/\d{4}-[^"]+baseball[^"]*)"/gi;
  let match;
...
function parseSetMetadata(url: string, html: string) {
  ...
  if (lower.includes('panini') || lower.includes('prizm') || lower.includes('donruss') || lower.includes('select') || lower.includes('mosaic') || lower.includes('optic')) {
    manufacturer = 'Panini';
  } else if (lower.includes('topps') || lower.includes('bowman') || lower.includes('chrome') || lower.includes('heritage') || lower.includes('gypsy') || lower.includes('allen')) {
    manufacturer = 'Topps';
  ...
  let productLine = '';
  const patterns = ['topps', 'bowman', 'chrome', 'heritage', 'gypsy-queen', 'allen-ginter', 'archives', 'stadium-club', 'tier-one', 'tribute', 'prizm', 'select', 'donruss', 'optic', 'diamond-kings'];
...
async function scrapeYear(year: string) {
  console.log(`\n=== Scraping ${year} baseball sets ===`);
  const yearUrl = `${BASE_URL}/sports-cards-sets/mlb-baseball-cards/${year}-baseball-cards`;
  ...
  for (const setUrl of setUrls) {
    ...
    if (existingSet) {
      ...
      const { data: newSet, error } = await supabase.from('card_sets').insert({
        name: metadata.name, year: metadata.year, sport: 'baseball',
...
async function main() {
  console.log('Baseball Checklist Scraper\n');
